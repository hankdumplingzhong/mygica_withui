...
class ScriptConfig:
    ...
    video_preset: list[str] = field(default_factory=lambda: ['-c:v', 'hevc_nvenc', '-cq', '23', '-sn', '-dn'])
    ...
    def __post_init__(self):
        assert self.toml_path.suffixes == ['.MyGICA', '.toml'], 'need .MyGICA.toml file'
        assert self.toml_path.exists(), 'toml path should exists'
...
# 缓存剪辑
# =============================
def cache_clip(cmd: list[str]) -> Path:
...
def work(config: ScriptConfig) -> None:
    ...
    # =============================
    # add_bgm.main(Path(project.sources['bgm']), frame_to_time(project.start, project.fps), config.output)
...
def cat_video(output: Path, segment_files: list[Path], config: ScriptConfig, param: list[str]) -> None:
    ...
    cmd = [
              ...
              # '-fps_mode', 'cfr',
              # '-c', 'copy',
              '-vsync', 'cfr',  # Add this to enforce a constant frame rate
              '-r', str(config.project.fps),  # Explicitly set the output frame rate
          ] + param + [
              # ] + config.video_preset + [
              # '-fflags', '+genpts',
              # '-video_track_timescale', '24000',
    ...
    for i, clip in enumerate(rng.clips):
        ...
        if is_image(Path(src_path)):
            ...
            cmd = [
                      ...
                      '-i', 'anullsrc',
                      '-t', frame_to_time(frame_count, config.project.fps),  # 设置音频时长与视频匹配
                      '-loop', '1',
                      '-i', src_path,
