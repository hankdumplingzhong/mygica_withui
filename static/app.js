...
function drawTimeline(model) {
    ...
    const fps = Number(model.project.fps_float || 0); // 用后端解析好的浮点 fps
...
async function loadFile() {
    const name = $('#fileSelect').value; if (!name) return;
...
async function build() {
    ...
    const r = await api('/build/full', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ file: currentFile }) });
    log('开始构建: ' + r.cmd);
...
function setupMonaco() {
    require(['vs/editor/editor.main'], function () {
        ...
        editor.onDidChangeModelContent(() => { /* 轻量防抖 */ if (window._pt) clearTimeout(window._pt); window._pt = setTimeout(parseNow, 500); });
    });
}
